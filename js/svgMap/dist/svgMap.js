...
svgMap.prototype.createMap = function () {
  ...
  Object.keys(mapPaths).forEach(function (countryID) {
    ...
    /* SPOTIFY_RAMPUP_EDIT original #d9ecff; */
    countryElement.addEventListener('click', function () {
      var countryID = countryElement.getAttribute('data-id');
      console.log(countryID);
      
      /* FRANK OCEAN */
      console.log("spotifyID: " + this.options.data.values[countryID]["spotifyID"]);
      const spotifyLink = this.options.data.values[countryID]["spotifyID"];
      window.open("https://open.spotify.com/playlist/" + spotifyLink, '_blank');
    }.bind(this));
